...
func TestClientApplyConfigPushRejectsInvalidSnapshotsWithoutAckOrRuntimeTeardown(t *testing.T) {
	cases := []struct {
		...
		{
			name: "unbound tunnel with non-zero rate policy fields",
			push: protocol.ConfigPush{
				ConfigVersion: 2,
				GeneratedAtMs: 200,
				Tunnels: []protocol.TunnelEntry{
			...
			wantMessage: "unbound tunnel rate policy fields must be zero",
		},
		{
			name: "bound tunnel with unsupported rate policy mode",
			push: protocol.ConfigPush{
				ConfigVersion: 2,
				GeneratedAtMs: 200,
				Tunnels: []protocol.TunnelEntry{
			...
			wantMessage: "unsupported rate policy mode",
		},
		{
			name: "bound range tunnel with rate policy",
			push: protocol.ConfigPush{
				ConfigVersion: 2,
				GeneratedAtMs: 200,
				Tunnels: []protocol.TunnelEntry{
			...
			wantMessage: "rate policy is only supported for single-port tunnels",
		},
		{
			name: "bound tunnel with zero rate policy bps",
			push: protocol.ConfigPush{
				ConfigVersion: 2,
				GeneratedAtMs: 200,
				Tunnels: []protocol.TunnelEntry{
			...
			wantMessage: "rate policy downlink and uplink must be non-zero",
		},
	}
