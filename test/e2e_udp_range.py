...
class Ports:
    ...
    def remote_range_end(self) -> int:
        return self.remote_range_start + UDP_RANGE_SIZE - 1

    @property
    def local_range_end(self) -> int:
        return self.local_range_start + UDP_RANGE_SIZE - 1

    @property
    def remote_range_ports(self) -> list[int]:
        return [self.remote_range_start + offset for offset in range(UDP_RANGE_SIZE)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a minimal UDP range e2e test against local frps/frpc with Python public "
            "clients and Python local UDP servers."
        ),
    )
    parser.add_argument(
        "--frps-bin",
    ...
    parser.add_argument(
        ...
        help=f"Timeout in seconds for readiness checks and UDP round-trips. Default: {DEFAULT_TIMEOUT_SECONDS}.",
    )
    parser.add_argument(
        "--frps-log-level",
        default="info",
...
def main() -> int:
    ...
    try:
        ...
        stage = "seed udp single and range runtime data"
        print(f"[stage] {stage}")
        seed_runtime_data(paths.db_path, token, ports)

        stage = "start python local udp servers"
        print(f"[stage] {stage}")
        local_servers = [
            start_recorded_udp_server("single", "127.0.0.1", ports.local_single, b"single:"),
            start_recorded_udp_server("range-0", "127.0.0.1", ports.local_range_start, b"range-0:"),
            start_recorded_udp_server("range-1", "127.0.0.1", ports.local_range_start + 1, b"range-1:"),
        ]
        ...
        stage = "wait for udp single and range listeners"
        print(f"[stage] {stage}")
        ...
        result = {
            ...
            "verified_steps": [
                ...
                "sqlite seed for one udp single tunnel and one udp range tunnel",
        ...
        for handle in local_servers:
            stop_recorded_udp_server(handle)
...
def allocate_ports() -> Ports:
    ...
    local_range_start = find_free_udp_port_range(UDP_RANGE_SIZE, excluded)
...
def find_free_udp_port(excluded: set[int]) -> int:
    for _ in range(200):
    ...
    raise RuntimeError("failed to allocate a distinct udp port")


def find_free_udp_port_range(size: int, excluded: set[int]) -> int:
    if size <= 0:
        raise ValueError("udp port range size must be positive")

    seed = find_free_udp_port(excluded)
    max_start = 65535 - size + 1
    for start in range(max(seed, 1024), max_start + 1):
        if udp_range_is_bindable(start, size, excluded):
            return start
    for start in range(1024, max(seed, 1024)):
        if udp_range_is_bindable(start, size, excluded):
            return start

    raise RuntimeError(f"failed to allocate contiguous udp port range of size {size}")


def udp_range_is_bindable(start: int, size: int, excluded: set[int]) -> bool:
...
def start_recorded_udp_server(name: str, host: str, port: int, response_prefix: bytes) -> RecordedUDPServerHandle:
    ...
    def serve() -> None:
        while not handle.stop_event.is_set():
            ...
            try:
                sock.sendto(handle.response_prefix + payload, addr)
            except OSError as exc:
                with handle.lock:
                    handle.error = str(exc)
                return

    handle.thread = threading.Thread(target=serve, name=f"{name}-udp-server", daemon=True)
    handle.thread.start()
    return handle


def stop_recorded_udp_server(handle: RecordedUDPServerHandle) -> None:
...
def run_verification_round(
    ...
    single_response = run_public_udp_client("127.0.0.1", ports.remote_single, single_payload, timeout_seconds)
    expected_single = single_handle.response_prefix + single_payload
    if single_response != expected_single:
        raise RuntimeError(f"single udp response mismatch: sent {single_payload!r}, received {single_response!r}")
    wait_for_server_payload(single_handle, single_payload, timeout_seconds)
    ...
    for index, payload in enumerate(range_payloads):
        ...
        response = run_public_udp_client("127.0.0.1", remote_port, payload, timeout_seconds)
        expected = handle.response_prefix + payload
        if response != expected:
            raise RuntimeError(
                f"range udp response mismatch for remote port {remote_port}: sent {payload!r}, received {response!r}"
            )
    ...
    range1_received, range1_clients, range1_error = handles_by_name["range-1"].snapshot()
    for server_name, error_text in [
        ("single", single_error),
        ("range-0", range0_error),
        ("range-1", range1_error),
    ]:
    ...
    return {
        ...
        "single_server_client_addrs": [f"{host}:{port}" for host, port in single_clients],
        "range_server_client_addrs": [
            [f"{host}:{port}" for host, port in range0_clients],
            [f"{host}:{port}" for host, port in range1_clients],
        ],
    }
...
def wait_for_server_payload(handle: RecordedUDPServerHandle, payload: bytes, timeout_seconds: float) -> None:
    ...
    raise TimeoutError(f"local udp server {handle.name} did not observe payload in time: {payload!r}")


def ensure_exact_server_payloads(handle: RecordedUDPServerHandle, expected_payloads: list[bytes]) -> None:
    received_payloads, _, error_text = handle.snapshot()
    if error_text is not None:
        raise RuntimeError(f"local udp server {handle.name} failed: {error_text}")
