...
### 4.3 当前已实现的统一证书 / CA 资产层

本节描述当前已经落地到代码、schema 和管理 API 的证书资产模型。
...
- 只有 `source=generated` 且 `asset_type=ca` 且 `key` 非空的资产，当前才能作为签发 CA 使用
...
“导入现有证书或 CA”当前同时支持两种输入方式：

- 上传 `crt` / `key` 文件
- 手动粘贴 `crt` / `key` PEM 文本

这两种方式最终都进入同一条资产导入路径，统一落成 `source=upload`。
...
其中导入路径由后端根据首张证书自动识别 `asset_type`；生成路径由请求体显式指定 `asset_type`。
...
- 自签根 CA 只写资产表，不写上游关系
- 生成 CA 和生成证书当前都支持 `key_algorithm` + `key_bits`
- 当前支持的算法为 `ecdsa`、`rsa`、`ed25519`
- `ecdsa` 当前允许 `256 / 384 / 521`
- `ed25519` 当前固定为 `256`
- `rsa` 当前允许管理员输入任意 `>= 2048` 且为 `8` 的倍数的长度
...
- 生成资产默认只下载当前节点
- 生成资产允许按单父树关系做“当前节点到某个祖先”的单条链下载；输出为 ZIP，内部包含一个聚合后的 `*.crt`，有私钥时额外带当前节点 `*.key`
- 生成资产允许按树关系下载某个 CA 节点的部分子树或整棵树；输出为内存打包后的 ZIP，每个节点各自保存 `crt`，有私钥的节点额外带自己的 `key`
- 上传资产只允许下载当前资产原始结构；不支持补数据库上游、不支持裁剪链、不支持树状选择
- 打包和 `crt` 聚合统一在 `frps` 内存完成，不落盘，不调用外部工具
- 当前所有下载模式的 HTTP 响应外层统一都是 ZIP，不直接返回裸 PEM

删除语义当前固定为：

- 删除前先计算 `delete-impact`
- 如果某生成资产在 `certificate_asset_relations` 中被其他生成资产作为直接上游引用，则返回受影响资产列表并要求显式级联确认
- 如果删除某条 CA 后会导致其他上传 / 生成资产失去“数据库资产补链”能力并校验失败，这些资产也会进入受影响列表
- 级联删除时，目标资产和全部受影响资产会在同一事务内一起删除
...
- 登录 `key` 重置和分组删除不走热重载，继续直接断开当前控制连接。
