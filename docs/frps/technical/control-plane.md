...
当前控制面在真正打开 `7000` 前，已经先执行过一次 `EnsureInitialRuntimeScan()`。
...
原因不是“没有证书”，而是当前业务协议首包就是 `auth.begin`，还没有单独的安全模式协商层。

下一步设计见 [../design/certificate-binding.md](../design/certificate-binding.md)：

- 先加 `transport.client_hello / transport.server_hello`
- 再根据协商结果决定是否 TLS
- TLS 成功后才进入现有 `auth.begin` 链路
