...
当前实现不单独维护 `frps/migrations/` 目录，而是把 schema 定义和迁移步骤直接内嵌在服务端代码中。

当前约定：

- 数据库连接由上层 `internal/app/database.go` 按配置打开。
- `internal/storage/sql.go` 只封装数据库对象，不负责驱动打开和迁移。
- schema 版本、建表语句、校验规则统一维护在 `internal/app/schema.go`。
- 服务启动时先执行 `schema_migrations` bootstrap，再按版本顺序执行内嵌 migration。
- migration 完成后必须继续做严格表结构校验；如果现有表结构与内置定义不一致，服务立即退出。
- SQLite 和 MySQL 的 schema 要保持同一业务语义，但允许使用各自稳定的公共类型映射。
- 空库允许自动初始化，非空库不允许“带病启动”。

后续新增表或字段时，开发流程固定为：

1. 在 `internal/app/schema.go` 中同时更新 SQLite/MySQL 的 migration 语句。
2. 同步更新对应的表结构校验定义。
