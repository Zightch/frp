...
当前“证书管理”已经先收口出一层明确设计，但还没有进入真实代码链路；这一步只定义证书资产，不定义后续绑定关系。

当前已确认的范围如下：

- `frps` 后续会管理一个证书资产集合，可同时保存多张证书
- 一条证书资产固定是一组 `crt + key`
- 当前只支持 `pem` 格式
- 证书原文直接存数据库
- 管理员通过 WebUI 管理证书资产
- 导入现有证书时，同时支持“上传文件”和“手动粘贴 PEM”
- 管理面还支持“自签证书”后直接入库
...
- `source` 当前只区分 `upload` 和 `self_signed`
- 文件上传与手动粘贴都属于导入现有证书，统一记为 `upload`
- `type` 当前固定为 `pem`
...
后续反向代理、WebUI HTTPS、监听器等模块只应“选择证书资产”，但怎么选择、在哪一层绑定，留到后续单独设计。
