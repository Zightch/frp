...
当前 WebUI 只开放下面这些真实页面与结构：

- `/init` 只在未初始化态可进入；只负责一次性写入首个管理密钥
- `/login` 是极简 challenge 登录页，默认只保留标题、管理密钥输入框、登录按钮和动态报错
- 已登录管理态的一级导航只有 `/` 和 `/proxy-groups`
- `/proxy-groups` 是统一接入管理主入口：上半区负责分组 CRUD 与 token 重置，下半区只展示当前选中分组下的隧道 CRUD
- `/tunnels` 只保留兼容路径，复用同一张接入管理页，并高亮到“接入管理”
- 管理壳层统一承接侧边导航、顶部会话状态、全局错误槽和通用页面骨架
- 管理面不展示未来功能导航；概览页只做状态总览和快捷入口
...
当前 WebUI 还固定了下面这些上下文约束：

- 接入管理页只展示当前选中分组下的隧道，不再展示跨分组全量隧道主表
- 新建隧道时，`group_id` 默认绑定到当前选中分组，不在该页里跨分组切换
