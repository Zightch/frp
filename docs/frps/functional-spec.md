...
下面这些规则已经确认，其中 `frps` 侧整组冻结、pending 重载状态管理和 `ack` 后 listener 重建已落地；`frpc` 侧本地资源清理与快照替换仍待完成：
...
- 当前仍缺 `frpc` 在 `ack` 前完成本地资源清理和运行态替换；因此当前 `config.ack(status=ok)` 还不能对外承诺“客户端运行态已完全切换完成”。
