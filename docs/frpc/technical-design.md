...
1. 解析完整快照
2. 原子替换 `sessionState.snapshot`
3. 返回 `config.ack`
4. 更新 `lastAckedConfigVersion`

当前边界：

- 配置只在登录阶段接收
- 管理端改库后，当前代码不会收到服务端主动热更新
- 活跃 TCP/UDP 运行态不依赖“配置重放”继续工作

### 4.1 已确认待实现的在线整组配置热重载方案

下面这些规则已经确认，但当前仍属于待实现设计：

- `frpc` 后续可能在登录完成后再次收到新的 `config.push`；每次仍然是整组完整快照，而不是单 tunnel 增量事件。
- `frpc` 必须在本地对比旧快照和新快照，识别哪些 tunnel 是新增、删除或替换。
- tunnel 名称不在运行态快照内；因此单纯改名称不会触发 `frpc` 运行态变更。

首版行为边界固定为：

- 服务端会按“整组冻结 + 整组全量重建”推进热重载，因此 `frpc` 首版不要求保留未受影响 tunnel 的活动本地连接。
- `frpc` 在返回 `config.ack(status=ok)` 之前，必须先完成本地资源清理和新快照替换。
