...
当前不承担管理、策略、观测或反向代理职责；默认不承载任何非必要的权威功能。
...
- 登录冲突时直接退出，不进入重连
- 本地拨号或本地读写失败后的错误回传
- 启用 tunnel 启动日志统一输出 `tls:listen=<mode>,backend=<mode>` TLS 摘要
...
- 如果登录阶段收到服务端返回的非重试错误，例如同 `proxy_group` 已有其他 `frpc` 在线，则直接退出，不进入重连；错误消息会带当前在线 `frpc` 的 `ip:port`。
