...
- `frps` 第一阶段框架已落地，包含启动入口、配置加载、结构化日志、管理端 HTTP 健康检查、`7000` 控制端口监听骨架。
- 数据库基建已落地，支持 SQLite/MySQL、统一 `storage.SQL` 封装、事务封装、启动期内嵌 schema bootstrap 与严格校验。
- `frpc`、控制协议落地、真实代理链路、WebUI 业务页仍处于后续开发阶段。

这些文档现在同时承担两类职责：

- 固定中长期设计边界，避免后续开发时职责漂移。
- 记录当前已经落地的实现方式，避免文档和代码状态脱节。
