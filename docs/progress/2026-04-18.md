...
以下归档承接 2026-04-18 当天为主的实现进度；个别最早条目如果时间点不够精确，已按相邻提交时间近似归档。
...
实现原则保持极简核心优先：只补必要诊断信息，不引入日志框架、测试 DSL 或额外模块拆分。

## 4. 当时停止线

这一轮在 2026-04-18 的停止线明确如下：

- 不继续扩展测试矩阵
- 不拆 Python 脚本模块
- 不引入额外日志框架、额外状态机、额外测试 DSL
- 不把在线改库热更新、ACL、多客户端扩展、反向代理、UDP、端口范围、抓包、限速并入已完成主线
...
- 页面只管理两个对象：`proxy_groups`、`tunnels`。
- 分组支持列表、新增、编辑、删除、创建或重置 token。
- 隧道支持列表、新增、编辑、删除。
...
- 下一轮方向改为“最小配置生效闭环”
- 原则是优先复用现有 `frpc` 自动重连、登录和 `config.push` 下发链路
- 倾向的最小实现方式是：管理面写库后定位受影响 group，若该 group 当前有在线 `frpc` 会话，则由 `frps` 主动回收该会话，让 `frpc` 自动重连重新领取配置
