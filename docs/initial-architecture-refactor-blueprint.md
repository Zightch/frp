...
历史说明：本文是早期拆分蓝图，用于追溯当时的重构方向，不作为当前代码结构口径。当前 `frps` 控制面结构以 [frps/frpc 当前代码架构图](frps-frpc-current-architecture.md) 和 [frps 控制面技术设计](frps/technical/control-plane.md) 为准。

本文是当前这一轮“初步框架重构”的冻结蓝图。它只定义本轮必须落地的结构收束方案、迁移顺序和不变边界，不承载长期能力愿景。

如果本文与当前正式文档冲突，以正式文档和当前代码为准。
...
- 本轮最终确认唯一值得继续共享的新增纯规则是 challenge 响应摘要，现已收口到 `pkg/protocol.ChallengeResponse`；其余候选继续保持两端局部实现。
