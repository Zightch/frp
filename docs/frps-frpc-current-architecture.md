...
- UDP 当前已经具备 `frps` 公网 listener、控制帧桥接和 `frpc` 本地 UDP 转发；剩余缺口是 `frps` 侧空闲 `30s` cleanup 和最小 e2e 验证。
...
- 因此，UDP 当前属于“公网入口和本地转发已闭环、`frps` `30s` idle cleanup 尚未收口”的状态；端口范围、隧道入口 ACL、限速、抓包、在线观测等能力仍未进入真实执行链路。
