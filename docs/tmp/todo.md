...
- 启动门闩、管理 API 首次可见内容、未监听轮询补启动、首次握手快照变更、首次 `config.ack` 后立即故障边界、异常输入矩阵、“空配置保活后的客户端断线重连”、“外部占用高频抖动同一端口”，以及“`effective_ip` 在本机地址集合内外高频抖动”都已完成并归档；当前 `todo` 只保留剩余测试任务，不回填已完成项。
