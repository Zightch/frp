...
- 继续为端口冲突检查 / 热更新交互补齐正式测试矩阵；当前确定性场景、异常输入矩阵，以及部分竞争窗口与恢复故障场景已补齐，后续把剩余测试任务按“恢复路径、稳定性入口”继续推进。
...
- 启动门闩、管理 API 首次可见内容、未监听轮询补启动、首次握手快照变更、首次 `config.ack` 后立即故障边界、异常输入矩阵、“空配置保活后的客户端断线重连”，以及“外部占用高频抖动同一端口”都已完成并归档；当前 `todo` 只保留剩余测试任务，不回填已完成项。
