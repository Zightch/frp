...
- 关键竞态点盘点：逐一盘点首轮扫描、控制端口开放、管理 API 首次可见、`RefreshGroup()`、`scanNonListeningTunnelRuntimeIssues()`、`config.push`、`config.ack`、listener bind/freeze/close、session shutdown、本机网络快照刷新等路径的并发边界、共享状态和可插桩位置，形成明确的“哪里需要 hook / barrier / fake”的代码清单。
- 测试钩子骨架：设计统一的 test-only hook / barrier 注入方式，要求默认零开销、生产构建不可误用、单测可定点阻塞和放行，并能覆盖首轮扫描、热更新、轮询、会话切换和 listener 生命周期。
- 可控调度骨架：把轮询周期、超时、重试、心跳等待、恢复等待等真实时间依赖抽离成可手动推进的 clock / ticker / scheduler 接口，先消除 `sleep` 驱动的测试写法。
