...
- 继续为端口冲突检查 / 热更新交互补齐正式测试矩阵；当前确定性场景已全部补齐，后续把剩余测试任务按“异常输入、竞争窗口、恢复路径、稳定性入口”完整推进。
...
- 启动门闩、管理 API 首次可见内容、未监听轮询补启动、首次握手快照变更、首次 `config.ack` 后立即故障边界等确定性场景已完成并归档；当前 `todo` 只保留剩余测试任务，不回填已完成项。
