...
│  生效 IP: 0.0.0.0 | frpc TLS: 启用 [重置密钥][编辑][删除] │
├────────────────────────┬────────────────────────────────────┤
...
- 点击分组表格行后，左侧表格行高亮。
- 右侧隧道列表立即切换为当前分组的数据。
- 顶部操作栏同步显示当前选中分组名称、状态、`client_id`、`effective_ip` 和 `frpc TLS`。
...
- 当前若还没有绑定 `frpc_tls`，表单会展示提示；真正保存校验以后端为准。
