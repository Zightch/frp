...
| `/rate-policies` | `RatePolicyView.vue` | 限速策略管理页（详见 `docs/webui/rate-policy.md`） |
| `/certificate-assets` | `CertificateAssetsView.vue` | 证书资产管理页 |
...
- 证书资产管理
  - 支持列表查看、按类型/来源筛选。
  - 支持导入证书/CA（上传文件或粘贴 PEM）。
  - 支持生成 CA 和用 CA 签发证书。
  - 支持编辑资产元数据（仅名称、备注）。
  - 支持查看详情、下载和删除（带级联影响提示）。
- 基础交互
...
- 当前侧边栏有四个一级入口：”分组配置”、”限速策略”、”证书资产”和”系统设置”。
- “分组配置”页由三块组成：
  - 页面标题和刷新按钮
  - 当前选中分组的操作栏
  - 左侧分组列表与右侧隧道列表的双栏内容区
- “限速策略”页由三块组成：
  - 页面标题和刷新按钮
  - 当前选中策略的操作栏
  - 左侧策略列表（带模式筛选）与右侧绑定隧道列表的双栏内容区
- “证书资产”页由三块组成：
  - 页面标题和刷新按钮
  - 筛选栏（资产类型、来源筛选）
  - 资产列表表格
- 列表页都已经做了纵向撑满适配，底边会跟随浏览器可视高度变化。
- 当前主内容区设置了最小宽度，窄窗口下以内容区横向滚动为主，不等同于完整移动端适配。

## 5. 当前边界

- 不改后端 API、认证算法、Cookie 会话模型、数据库 schema 和控制协议。
- 限速策略页面已规划（详见 `docs/webui/rate-policy.md`），当前尚未实现。
- 不承载抓包开关这类仅属于运行时的控制。
