...
- challenge 登录
  ...
  - 管理密钥认证通过后，服务端还会在签发管理会话前检查当前是否已有在线管理员。
  - 如果当前已有管理员在线，登录页进入拒绝等待态，并显示显式“顶掉”入口，而不是直接接管。
- 会话状态检查与退出登录
...
- 限速策略管理
  - 支持列表、新建、编辑、删除。
  - 支持查看当前策略绑定隧道。
  - 页面交互已收口为独立限速策略 + 绑定编辑对话框模型，细节见 `docs/webui/rate-policy.md`。
- 基础交互
...
- 不改认证算法、Cookie 会话模型、数据库 schema 和控制协议。
- 当前 WebUI 交互按“单管理员浏览器会话”模型设计：
  - 管理登录必须先通过管理密钥认证，不能在未通过认证前直接参与占用判断。
  - 管理密钥认证通过后，如果当前已有管理员在线，则当前页面只收到“拒绝登录，可选顶掉”的等待态结果。
  - 等待态页面不实时感知目标占用者下线；顶掉按钮不会自动禁用、自动切换为“立即登录”，也不会因为空位出现而自动成功。
  - 每个等待态都只绑定自己看到的那一版管理员占用事实；顶掉只能替换自己看到的那个占用者。
  - 如果等待页点击顶掉时，目标占用者已经下线或已经变成别人，则页面统一提示“页面已失效，请重新登录”。
  - 这个拒绝结果不形成排队，不保留等待资格，也不在当前管理员下线后自动把管理位发给先前被拒绝的页面。
  - 当前管理员下线后，管理位直接回到空闲；旧等待页不能靠陈旧的顶掉请求抢空位，必须重新登录。
  - 当前阶段不为多浏览器并发编辑提供协同机制。
- 限速策略页面已接入，页面细节和边界见 `docs/webui/rate-policy.md`。
- 不承载抓包开关这类仅属于运行时的控制。
