...
- **顶部导航栏**：品牌标识 + 用户操作区（退出登录）。固定高度 56px，背景白色，底边框 1px `--color-border-light`。
- **侧边栏**：导航菜单，宽度 200px，背景白色，右边框 1px `--color-border-light`。当前页高亮使用品牌色。
- **主内容区**：背景 `--color-bg-page`，使用全宽内容区，基础内边距 24px。
- **可视高度适配**：主框架使用 `100vh/100dvh` 计算剩余高度，页面内部列表区继续通过 `flex` 和表格 `height="100%"` 撑满可用空间。
- **横向边界**：主内容区当前保留 `920px` 最小宽度，窄窗口下优先出现横向滚动，不做自动折叠布局。

### 3.2 登录页布局

独立全屏居中卡片，不受侧边栏约束（当前已实现）。

- 卡片容器最大宽度 400px，白色背景，8px 圆角，1px `--color-border` 边框
...
当前 WebUI 以桌面端管理台为主，已实现的是“纵向可用高度自适应”，不是完整响应式改造。

| 场景 | 当前行为 |
|------|----------|
| 高度变化 | Header、Aside 固定；主内容区和页面内列表区会跟随窗口高度自动伸缩 |
| 宽度充足 | 维持左侧分组列表 + 右侧隧道列表双栏布局 |
| 宽度不足 | 主内容区保持最小宽度，浏览器出现横向滚动 |
| 移动端 | 当前未单独设计折叠菜单、抽屉导航或单栏重排 |
...
- 选中行背景 `--color-primary-light-9`。
- 边框使用 `--color-border-lighter`，不使用粗边框。
- 行可点击时显示 `cursor: pointer`。

### 6.3 表单

- 标签字号 14px，颜色 `--color-text-regular`，右对齐（当前主要使用 `80px` 或 `100px`）。
...
- 页面级：当前主要表现为居中文本状态，可按页面需要补充 spinner。
...
- 登录成功后跳转管理页。
- 如果登录后发现当前已有管理员在线，则停留在认证页内的拒绝等待态，由用户显式决定是否顶掉。
- 等待态点击顶掉时，如果目标占用者已经下线或已经变成别人，则提示“页面已失效，请重新登录”，并回到重新登录流程。
