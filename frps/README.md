...
应用启动时会在上层按配置打开数据库，并自动执行内置 schema bootstrap 与校验。
当前第一阶段内置的核心表包括：
...
空库会在启动时自动建表；如果现有 SQLite/MySQL 表结构与服务内置 schema 不一致，`frps` 会直接退出，避免带着错误库结构继续运行。
...
- `internal/app/schema.go`：schema 版本、建表、校验
