...
func TestServerScenarioKeepsSessionAliveWhenTCPRecoveryBindFailsWithPermissionDenied(t *testing.T) {
	...
	expectedReason := controlruntime.BuildTunnelListenerStartReason(protocol.ProtocolTCP, listenKey.IP, listenKey.Port, syscall.EACCES)
...
func TestServerScenarioClosesPartiallyStartedTCPListenersAcrossRepeatedEMFILERecoveryChurn(t *testing.T) {
	...
	expectedReason := controlruntime.BuildTunnelListenerStartReason(protocol.ProtocolTCP, failingKey.IP, failingKey.Port, syscall.EMFILE)
