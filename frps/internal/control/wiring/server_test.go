...
func TestServerClosesPublicTCPConnectionWhenNoIdleWorkConn(t *testing.T) {
	var tokenID [16]byte
	copy(tokenID[:], []byte("token-id-1234567"))
	...
	controlConn, controlDone, _, configFrame := loginControlSessionForTCPWorkTest(t, server, tokenID, tokenHash)
	defer controlConn.Close()
	...
	if frame, err := readMessageWithin(controlConn, 200*time.Millisecond); err == nil {
		t.Fatalf("unexpected control frame without idle work conn: %s", frame.Type.String())
	} else if !isTimeoutError(err) {
		t.Fatalf("unexpected control connection result without idle work conn: %v", err)
	}

	waitForReadSideClose(t, publicConn)

	deadline := time.Now().Add(time.Second)
	for {
		_, runtimeState := active.session.ObserveState()
		if runtimeState.ActiveStreamCount == 0 {
			break
		}
		if time.Now().After(deadline) {
			t.Fatalf("expected no active tcp streams after work-pool miss, got %d", runtimeState.ActiveStreamCount)
		}
...
func waitForReadSideClose(t *testing.T, conn net.Conn) {
	t.Helper()
